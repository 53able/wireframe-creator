...
7. ブラウザ接続の確認後に、要件、ユーザーストーリー、画面仕様、既存UI、対象デバイスを詳しく調査する。指定ファイルを最優先し、`input` を `pass`、`context` を `running` へ更新する。
8. `references/context-interview.md` を読み、問題と範囲、対象ユーザー、ユーザー成果、ビジネス成果、ソリューション境界、主要シナリオ、仮説、最大リスク、実験範囲、提供制約のコンテキスト台帳を作る。入力資料から判断できる項目を先に埋め、観測事実と推論を分ける。
9. 欠落すると対象ユーザー、主要フロー、検証目的、またはスコープが大きく変わる項目がある場合は、同プロトコルに従って最大6問を番号付きの1メッセージへまとめて逆質問する。冒頭で、分からない項目は「不明」でよく、その場合は提供資料から推論して仮定として明記すると伝える。保存場所、既存正本の上書き可否、矛盾する要求は推論しない。質問した場合は、回答を受けるまで `context` を `running` のままにする。
10. 回答後または質問不要の場合は台帳を更新する。不明または無回答の項目は、同プロトコルの既定に従って最も保守的で検証可能な案を推論し、根拠と確信度を付ける。壁打ちでは、複数案が同程度に成立する場合に推奨案、代替案、今回決める事項を提示する。`context` を `pass`、`brief` を `running` へ更新する。可視ブラウザでHTMLを開いていない場合は、リアルタイム表示または経過時間を目視確認したと主張しない。

### Step 2: 検証目的と最小オラクルを定義する

1. `references/wireframe-principles.md` とStep 1のコンテキスト台帳を読む。
2. 台帳から対象ユーザー、課題、期待するユーザー成果、期待するビジネス成果、仮説、最大リスク、今回学ぶことを各1件定義する。推論した項目は仮定として扱い、正本HTMLの「前提」へ記載する。
...
- 対象ユーザーまたは主要タスクが特定できない場合は、空の画面群を作らない。`references/context-interview.md` に従って逆質問し、回答が不明なら入力資料から最も保守的な案を推論して、前提と確信度を明記する。
